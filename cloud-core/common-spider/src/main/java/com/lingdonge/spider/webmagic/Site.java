...
public class Site {
    ...
    /**
     * 添加一个长的Cookie字符串，不用自己再一个个解析了
     *
     * @param cookies
     * @return
     */
    public Site addCookie(String cookies) {

        Map<String, Cookie> listCookies = CookieUtil.parseCookies(cookies);

        listCookies.forEach((key, cookie) -> {
            defaultCookies.put(cookie.getName(), cookie.getValue());
        });
        return this;
    }
